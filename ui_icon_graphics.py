...
class IconGraphics(QtWidgets.QWidget):
    ...
    def set_position(self, x, y):
        self.setGeometry(x * (ICON_SIZE_PX + self.r_margin), y * (ICON_SIZE_PX + self.y_margin),
                         ICON_SIZE_PX + self.r_margin, ICON_SIZE_PX + self.y_margin)
    ...
    def show_top_text(self, string):
        ...
        x, y = (
                           ICON_SIZE_PX - boundingRectangle.width()) // 2, self.y_margin + 12 - boundingRectangle.height() // 2  # 21 21 the center
        text.setPos(x, y);
        self.set_shadow()
    ...
    def show_bottom_text(self, string):
        ...
        boundingRectangle = text.sceneBoundingRect()
        x, y = (
                           ICON_SIZE_PX - boundingRectangle.width()) // 2, self.y_margin + 30 - boundingRectangle.height() // 2  # 21 21 the center
        text.setPos(x, y);
        self.set_shadow()
        self.scene.addItem(text)
    ...
    def set_shadow(self):
        if not self.b_shadowed:
            self.scene.addRect(ICON_IMG_POS_X, ICON_IMG_POS_Y + self.y_margin, ICON_IMG_SIZE_PX, ICON_IMG_SIZE_PX,
                               invisible_pen, idle_counter_rect_brush)
    ...
    def show_idle_time_text(self, string):
        ...
        self.scene.addRect(0, 0, IDLE_COUNTER_WIDTH, IDLE_COUNTER_HEIGHT, idle_counter_rect_pen,
                           idle_counter_rect_brush)
        # Adds the text
        self.scene.addItem(text)
